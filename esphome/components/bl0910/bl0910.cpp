...
void BL0910::write_register_(uint8_t addr, uint8_t data_h, uint8_t data_m, uint8_t data_l) {
  ...
  this->write_array(packet, sizeof(packet) / sizeof(packet[0]));
}

int32_t BL0910::read_register_(uint8_t addr) {
  uint8_t packet[6] = {0};

  this->write(BL0910_READ_COMMAND);
  this->write(addr);

  if (this->read_array((uint8_t *) &(packet[2]), 4) <= 0) {
    ESP_LOGE(TAG, "Read read error");
    return 0xFF000000;
  }
